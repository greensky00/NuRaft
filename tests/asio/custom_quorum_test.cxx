...
int mark_down_after_leader_election_test() {
    ...
    // Test phase:
    //   0: normal
    //   1: S2 and S3 refuse requests from S1.
    //   2: S1 refuses requests from S2 and S3.
    ...
    i_opt.raft_callback_ = [&](cb_func::Type type, cb_func::Param* param)
        ...
        case 2:
            ...
            if (type == cb_func::Type::ReceivedAppendEntriesReq &&
                param && (param->peerId == 2 || param->peerId == 3)) {
                return cb_func::ReturnCode::ReturnNull;
            }
            break;
        default:
    ...
    // Same to S2 and S3, as S1 is isolated and unable to do full consensus.
    CHK_FALSE(s2.raftServer->is_part_of_full_consensus());
    CHK_FALSE(s3.raftServer->is_part_of_full_consensus());
